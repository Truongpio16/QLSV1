...
namespace QLsinhVien
{
    public partial class SinhVienForm : Form
    {
        ...
        private string connectionString = @"Data Source=.;Initial Catalog=QuanLySinhVien;Integrated Security=True";
        ...
        private void LoadDanhSachLop()
        {
            try
            {
                ...
                {
                    conn.Open();
                    string sql = "SELECT lqlma, lqlten FROM tbl_lopquanly";
                    SqlDataAdapter da = new SqlDataAdapter(sql, conn);
                    DataTable dt = new DataTable();
                    da.Fill(dt);

                    cmbLop.DataSource = dt;
                    cmbLop.DisplayMember = "lqlten";
                    cmbLop.ValueMember = "lqlma";
                }
            }
            catch (Exception ex)
            {
        ...
        private void LoadDanhSachSinhVien()
        {
            try
            {
                ...
                {
                    conn.Open();
                    string sql = @"SELECT svma AS 'Mã SV', 
                                          svten AS 'Họ tên', 
                                          svngaysinh AS 'Ngày sinh',
                                          CASE WHEN svgioitinh = 1 THEN 'Nữ' ELSE 'Nam' END AS 'Giới tính',
                                          svquequan AS 'Quê quán',
                                          lqlma AS 'Mã lớp'
                                   FROM tbl_sinhvien";
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string checkSql = "SELECT COUNT(*) FROM tbl_sinhvien WHERE svma = @maSV";
                    ...
                    // Thêm sinh viên
                    string sql = @"INSERT INTO tbl_sinhvien (svma, svten, svngaysinh, svgioitinh, svquequan, lqlma) 
                                   VALUES (@maSV, @tenSV, @ngaySinh, @gioiTinh, @queQuan, @maLop)";

                    using (SqlCommand cmd = new SqlCommand(sql, conn))
                    {
                        cmd.Parameters.AddWithValue("@maSV", txtMaSV.Text.Trim());
                        cmd.Parameters.AddWithValue("@tenSV", txtHoTen.Text.Trim());
                        cmd.Parameters.AddWithValue("@ngaySinh", dateTimePicker1.Value.Date);
                        cmd.Parameters.AddWithValue("@gioiTinh", radioButton1.Checked ? 0 : 1);
                        cmd.Parameters.AddWithValue("@queQuan", "");
            ...
            {
                MessageBox.Show("Lỗi thêm sinh viên: " + ex.Message, "Lỗi",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void btnSua_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    conn.Open();
                    string sql = @"UPDATE tbl_sinhvien 
                                   SET svten = @tenSV, 
                                       svngaysinh = @ngaySinh, 
                                       svgioitinh = @gioiTinh, 
                                       svquequan = @queQuan, 
            ...
            {
                MessageBox.Show("Lỗi sửa sinh viên: " + ex.Message, "Lỗi",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            DialogResult result = MessageBox.Show("Bạn có chắc muốn xóa sinh viên này?",
            ...
            if (result == DialogResult.Yes)
            {
                try
                {
                    ...
                    {
                        conn.Open();
                        string sql = "DELETE FROM tbl_sinhvien WHERE svma = @maSV";
